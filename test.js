...
function sostavChisla(massivChisel, chislo) {
  const result = [];
  massivChisel.sort();
  const backtrack = (index, currentTarget, current) => {
    if (currentTarget === chislo) {
      result.push(current);
    }
    if (currentTarget <= chislo) {
      for (let i = index; i < massivChisel.length; i++) {
        if (i > index && massivChisel[i] === massivChisel[i - 1]) {
          continue;
        }
        backtrack(i + 1, currentTarget + massivChisel[i], [...current, massivChisel[i]]);
      }
    }
  };
  backtrack(0, 0, []);
